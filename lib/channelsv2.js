...
for (const namespace of ['devices', 'session'])
  channels[namespace] = socket.of(`/${namespace}`)
...
channels.session.on('connection', async (socket) => {
  ...
  socket.on('rpc', async (data, ack) => {
    ...
    try {
      ack({ status: 'ok', data: await rpc(method, ...args) })
    } catch (err) {
...
const wrap = tag => async device =>
  channels.devices.emit(tag, new Device(device).valueOf())
...
export const broadcast = channels.session.emit.bind(channels.session)
